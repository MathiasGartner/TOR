...
class PickupProblems:
    ...
    def loadData(self, start, end):
        query = f"""
            SELECT  
                c.Position, 
                c.Latin,  
                CAST(TRIM(BOTH ' [' FROM SUBSTRING_INDEX(SUBSTRING_INDEX(l.Message, '[', -1), ',', 1)) AS DECIMAL(10,3)) AS X,
                CAST(TRIM(BOTH ' ]' FROM SUBSTRING_INDEX(SUBSTRING_INDEX(l.Message, ',', -1), ']', 1)) AS DECIMAL(10,3)) AS Y 
            FROM 
                clientlog l 
            LEFT JOIN 
                client c ON c.Id = l.ClientId 
            WHERE
                c.Position IS NOT NULL
                AND MessageType = 'WARNING'
                AND MessageCode = 'SAME_RESULT_N'
                AND Time > DATE_ADD(%(start)s, {ts.TIME_OFFSET_FOR_DISPLAY})
                AND Time < DATE_ADD(%(end)s, {ts.TIME_OFFSET_FOR_DISPLAY})
        """
        resultsRaw = DBManager.executeQuery(query, {"start": start, "end": end})
    ...
    def updatePlot(self, start, end, globalColorBar):
        print(f"Fetching data from '{start}' to '{end}'...")
        df = self.loadData(start, end)
        ...
        if globalColorBar:
            self.cbar = self.fig.colorbar(
                h[3], ax=axes,
                label=f"# of Problems (total {len(df)})",
                shrink=0.9
            )
